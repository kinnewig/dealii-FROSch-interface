...
namespace Step40
{
  ...
  template <int dim>
  void LaplaceProblem<dim>::setup_local_system()
  {
    local_dof_handler.distribute_dofs(fe);

    // TODO: The local problem is only sequentiel, but this is the typically parallel assembly
    IndexSet local_locally_owned_dofs    = local_dof_handler.locally_owned_dofs();
    IndexSet local_locally_relevant_dofs = DoFTools::extract_locally_relevant_dofs(local_dof_handler);

    // Remark: The local vectors only get the MPI_Comm of the current rank
    local_system_rhs.reinit(local_locally_owned_dofs, local_locally_relevant_dofs, MPI_COMM_SELF, true);

    local_constraints.clear();
    local_constraints.reinit(local_locally_relevant_dofs);
    DoFTools::make_hanging_node_constraints(local_dof_handler, local_constraints);
    ...
    SparsityTools::distribute_sparsity_pattern(dsp, local_locally_owned_dofs, MPI_COMM_SELF, local_locally_relevant_dofs);

    local_neumann_matrix.reinit(local_locally_owned_dofs, local_locally_owned_dofs, dsp, MPI_COMM_SELF);
    local_robin_matrix.reinit(local_locally_owned_dofs, local_locally_owned_dofs, dsp, MPI_COMM_SELF);
  ...
  template <int dim>
  void LaplaceProblem<dim>::assemble_local_system()
  {
    const QGauss<dim>     quadrature_formula(fe.degree + 1);
    const QGauss<dim - 1> quadrature_face_formula(fe.degree);
    ...
    for (const auto &cell : local_dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
      {
        ...
        {
          for (unsigned int j = 0; j < dofs_per_cell; ++j)
            cell_neumann_matrix(i, j) += fe_values.shape_grad(i, q_point) *
                                 fe_values.shape_grad(j, q_point) *
                                 fe_values.JxW(q_point);
        }
      }
      ...
      for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
      {
        ...
        for (unsigned int q_face_point = 0; q_face_point < n_q_face_points; ++q_face_point)
        {
          for ( const unsigned int i : fe_face_values.dof_indices() )
          {
            for ( const unsigned int j : fe_face_values.dof_indices() )
            {
              cell_robin_matrix(i, j) += 
                ( (alpha * fe_face_values.shape_value(i, q_face_point) * fe_face_values.shape_value(j, q_face_point) )
                  - (beta * fe_face_values.shape_grad(i, q_face_point) * fe_face_values.normal_vector(q_face_point) * fe_face_values.shape_value(j,q_face_point) )
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
        local_dof_indices[i] = optimized_schwarz_operator.get_dof(cell_counter, i);

      for (unsigned int i = 0; i < dofs_per_cell; ++i)
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
          {
            local_neumann_matrix.add(local_dof_indices[i], local_dof_indices[j], cell_neumann_matrix(i,j));
            local_robin_matrix.add(local_dof_indices[i], local_dof_indices[j], cell_robin_matrix(i,j));
          }
  ...
  void LaplaceProblem<dim>::solve()
  {
    ...
    LinearAlgebra::TpetraWrappers::SolverXpetra<double, Tpetra::KokkosClassic::DefaultNode::DefaultNodeType>
    ...
    // Preconditioner. This preconditioner can be selected with the two follwoing
    // lines:
    ...
    for (unsigned int cycle = 0; cycle < n_cycles; ++cycle)
      {
        ...
        optimized_schwarz_operator.compute(
