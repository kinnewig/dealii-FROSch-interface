...
namespace StepMaxwell
{
  ...
  template <int dim>
  MaxwellProblem<dim>::MaxwellProblem(std::string xml_file, MPI_Comm mpi_comm)
    : prm(xml_file)
    ...
    , fe(FE_NedelecSZ<dim>(
           prm.get_integer("Mesh and Geometry", "Polynomial degree")),
         2)
    , dof_handler(triangulation)
    , lambda(prm.get_double("Material Parameters", "Lambda"))
    , refrective_index(
        prm.get_complex_list("Material Parameters", "Refrective Index"))
  ...
  MaxwellProblem<dim>::make_grid()
  {
    ...
    std::vector<unsigned int> repetitions =
      (dim == 2) ? std::vector<unsigned int>{1, 3} :
                   std::vector<unsigned int>{1, 3, 1};
            ...
            for (unsigned int j = i; j < dofs_per_cell; j++)
              { // we are using the symmetry of the here
                ...
                std::complex<double> massterm =
                  (mu_term * curl_part) -
                  ((omega[cell->material_id()] * omega[cell->material_id()]) *
                   mass_part);
                    ...
                    for (const unsigned int j : fe_face_values.dof_indices())
                      {
                        ...
                        for (unsigned int q_point = 0;
                             ...
                             q_point++)
                          {
                            ...
                            robin -= imag_i * omega[cell->material_id()] *
                                     phi_i[q_point] * phi_j *
      ...
      switch (dim)
        {
          case 2:
            {
              MaxwellProblem<2> maxwell_problem("step-maxwell.xml",
                                                MPI_COMM_WORLD);
              maxwell_problem.run();
          ...
          case 3:
            {
              MaxwellProblem<3> maxwell_problem("step-maxwell.xml",
                                                MPI_COMM_WORLD);
              maxwell_problem.run();
